...
const buildContext = ({ rows, cols, blocked, pieces, role, maxPlacements }) => {
  ...
  const placementCap = Math.min(
    Math.max(0, Math.floor(maxPlacements || MAX_PLACEMENTS)),
    availablePieces || MAX_PLACEMENTS
  );

  const baseScorePool = [];
  piecePool.forEach((entry) => {
    const limit = Math.min(entry.remaining, placementCap - baseScorePool.length);
    for (let i = 0; i < limit; i += 1) {
      baseScorePool.push(entry.baseScore);
    }
  });
  baseScorePool.sort((a, b) => b - a);
  unusedBaseScore = baseScorePool.slice(0, placementCap).reduce((sum, val) => sum + val, 0);
...
const runBacktracking = (context, timeLimitMs) => {
  ...
  const dfs = (occupiedMask, baseScore, unusedScore) => {
    ...
    const placementsUsed = placementsStack.length;
    const slotsRemaining = Math.max(0, maxPlacements - placementsUsed);
    const remainingPieces = piecePool.reduce((sum, entry) => sum + entry.remaining, 0);
    const scaledUnused =
      remainingPieces > 0 && slotsRemaining < remainingPieces
        ? (unusedScore * slotsRemaining) / remainingPieces
        : unusedScore;
    const optimisticBonus = futureBonusBound();
    const optimisticTotal = baseScore + scaledUnused + optimisticBonus;
    ...
    if (placementsUsed < maxPlacements) {
      ...
      for (let i = 0; i < placementList.length; i += 1) {
        ...
        dfs(
          ...
          baseScore + entry.baseScore,
          Math.max(0, unusedScore - entry.baseScore)
        );
    ...
    dfs(occupiedMask | targetBit, baseScore, slotsRemaining > 0 ? unusedScore : 0);
  };
...
const buildExactCoverMatrix = (context) => {
  ...
  pieceColumns.forEach((col, idx) => {
    addRow({
      label: `${col.entry.modifier} 미사용`,
      modifier: null,
      grade: null,
      baseScore: 0,
      shapeKey: null,
      cells: [],
      columns: [cellColumns.length + idx],
    });
  });
...
const runAlgorithmX = (context, timeLimitMs) => {
  const start = performance.now();
  const deadline = start + timeLimitMs;
  const { modifierPotential, maxPlacements } = context;
  ...
  const search = () => {
    ...
    for (let i = 0; i < rows.length; i += 1) {
      ...
      const willPlacePiece = Boolean(row.modifier);
      if (willPlacePiece && placementsUsed >= maxPlacements) {
        continue;
      }
...
const runCsp = (context, timeLimitMs) => {
  ...
  const dfs = (occupiedMask, baseScore) => {
    ...
    const basePotential =
      remainingPieces > 0 && slotsRemaining < remainingPieces
        ? (unusedBaseScore * slotsRemaining) / remainingPieces
        : unusedBaseScore;
    const optimistic = baseScore + basePotential + futureBonusBound();
    if (optimistic <= bestResult.totalScore) return;
...
export const solveSugarBoard = ({
  ...
  const context = buildContext({ rows, cols, blocked, pieces, role, maxPlacements });
  if (!context) {
